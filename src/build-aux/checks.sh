...
# Check if files in src/axuralis.gresource.xml are sorted alphabetically.
check_resources() {
    [ $tap -eq 0 ] && echo -e "$Checking src/axuralis.gresource.xml…"
    ...
    done < src/axuralis.gresource.xml
    ...
    for i in ${!files[@]}; do
        if [[ "${files[$i]}" != "${to_sort[$i]}" ]]; then
            [ $tap -eq 0 ] && echo -e "$error Found file '${files[$i]#src/}' before '${to_sort[$i]#src/}' in axuralis.gresource.xml"
            [ $tap -eq 1 ] && echo "# Sorting error: '${files[$i]}' before '${to_sort[$i]}' in axuralis.gresource.xml"
            ret=1
            break
        fi
    done
    ...
    if [[ ret -eq 1 ]]; then
        if [[ $tap -eq 0 ]]; then
            echo ""
            echo -e "  Checking src/axuralis.gresource.xml result: $fail"
            echo "Please fix the above issues"
            exit 1
        else
            echo "not ok 4 - axuralis.gresource.xml"
        fi
    else
        if [[ $tap -eq 0 ]]; then
            echo -e "  Checking src/axuralis.gresource.xml result: $ok"
        else
            echo "ok 4 - axuralis.gresource.xml"
        fi
    fi
}
